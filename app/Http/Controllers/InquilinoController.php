...
class InquilinoController extends Controller
{
    ...
    public function datos_personales($transaccion = "")
    {
        ...
        //             return redirect()->route('inquilino.roomies', $user->transaction);
        //         } else if ($user->fase == 4) {
        //             return redirect()->route('registro_completado', $user->transaction);
        //         }
        //     }
        // }
    ...
    public function referencias($transaccion = "")
    {
        if (Auth::user()) {
            $user = User::where('email', Auth::user()->email)->first();
            if ($user->hasRole('broker')) {
        ...
        return view('arendatario.referencias', compact('transaccion_user'));
    }

    public function roomies($transaccion = "")
    {
        // if (Auth::user()) {
        //     $user = User::where('email', Auth::user()->email)->first();
        //     if ($user->hasRole('broker')) {
        ...
        return view('arendatario.roomies', compact('transaccion_user'));
    }
}
